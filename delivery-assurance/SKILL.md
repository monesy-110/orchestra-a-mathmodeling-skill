...
description: "Orchestra 完成编译、版式、匿名、页数、引用与提交前合规验收。适用于提交质量验收。"
---
...
If any format checks fail, Orchestra should fix 论文正文.tex to match the template format before recompiling.
...
for f in 论文/章节/*.tex; do
    ...
    l=$(grep -ci 'RESULTS\.md\|ORCHESTRA\.md\|MODELING_REPORT\|PROBLEM_ANALYSIS\|latex_includes' "$f" 2>/dev/null || echo 0)
