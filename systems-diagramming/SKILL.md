...
description: "Orchestra 构建技术路线、方法关系、系统架构与流程示意资产。适用于系统逻辑制图。"
---

# 系统逻辑制图

> ⛔ **风格基线对齐 nature-figure**：本阶段仍由内建 DrawIO/TikZ 机制执行（nature-figure 不覆盖非数据流程图），但所有图形的表达风格与克制约束对齐 `nature-figure` 的 `static/core/stance.md`：白底、低饱和单一强调色、一中性族 + 一信号族 + 一强调族、直角矩形、无渐变/阴影/圆角框、无图内标题（caption 由 LaTeX 管理）、最终版心内文字 ≥5pt 字形下限、图表服务于科学逻辑而非装饰。nature-figure 的 OpenRouter AI 生成路线**禁止**用于技术路线图、流程图与模型架构图（与本阶段硬约束一致），仅显式用户请求的物理场景示意可走其 `references/openrouter-image-generation.md` 并完成内容级核验与披露边界。
...
if grep -qi 'comp_apmcm_zh' "$PLAN_DOC" ORCHESTRA.md 2>/dev/null; then

    DRAWIO_LANG="zh"

elif grep -qi 'MCM\|ICM\|APMCM\|comp_mcm\|comp_apmcm\|comp_certcup_en\|comp_shuwei_en\|语言.*English\|Language.*English' "$PLAN_DOC" ORCHESTRA.md 2>/dev/null; then
...
if [ -f 图表/技术路线图.drawio ]; then
    ...
    for ROUND in 1 2 3; do
        ...
        if [ $ROUND -lt 3 ]; then
            ...
            # Orchestra: 你必须在这里读取一个示例模板，修改 技术路线图.drawio，然后重新导出 PDF
...
**⛔ 关键：上面的 bash 脚本只是检测框架。Orchestra 在看到 `⛔ 不合格` 交付后，务必：**
