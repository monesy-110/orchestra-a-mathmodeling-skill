...
async def run_skill(
    ...
    (workspace / "ORCHESTRA.md").write_text(agent_md, encoding="utf-8")
    ...
    if returncode != 0:
        raise RuntimeError(f"Orchestra runner failed for {skill_name} with exit code {returncode}")
    return result
