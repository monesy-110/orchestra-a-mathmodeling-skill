...
- **⛔ Caption 分隔符必须是空格，不能是冒号。** 中文论文的图表标题格式是"图 1 xxx"而不是"图 1: xxx"。在 preamble 中必须有 `\captionsetup{labelsep=quad}` 或 `\captionsetup{labelsep=space}`。如果模板已有此设置则不要重复添加。如果 Orchestra 自己写 main.tex，必须在 `\usepackage{caption}` 后加 `\captionsetup{labelsep=quad}`
...
- **⛔ 正文中禁止出现元叙述和内部指令。** 下列内容绝对不可出现在论文正文中：
  ...
  - "计算结果.md"、"图表/*.json"、"ORCHESTRA.md"、"建模报告.md" 等文件名 → 这些是内部工作文件，不是论文内容
