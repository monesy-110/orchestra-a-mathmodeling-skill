...
# Orchestra 有时在封面 tabular 中把 \cline 写在文本位置而非行分隔符位置
echo "--- 修复封面 cline 问题 ---"
...
# 9.1 自动检测并报告未嵌入的图表（供 Orchestra 编译步骤修复）
echo "--- 未嵌入图表检测 ---"
UNEMBED_COUNT=0
UNEMBED_LIST=""
...
for f in "$PAPER_DIR"/论文正文.tex "$PAPER_DIR"/章节/*.tex; do
    ...
    # stats 模板封面特有占位符检查（不自动清理，必须由 Orchestra 手动替换）
    ...
    if [ "$USES_TITLE_CMD" = true ]; then
        if grep -q '\\title{' "$PAPER_DIR/论文正文.tex" 2>/dev/null; then
            ...
            if [ -z "$TITLE_CLEAN" ]; then
                ...
                FALLBACK_TITLE=""
                if [ -f "ORCHESTRA.md" ]; then
                    FALLBACK_TITLE=$(grep -oP '(?<=题目|赛题|title)[：:]\s*\K.+' ORCHESTRA.md 2>/dev/null | head -1 | sed 's/[[:space:]]*$//')
                fi
            ...
            FALLBACK_TITLE="数学建模竞赛论文"
            if [ -f "ORCHESTRA.md" ]; then
                FT=$(grep -oP '(?<=题目|赛题|title)[：:]\s*\K.+' ORCHESTRA.md 2>/dev/null | head -1 | sed 's/[[:space:]]*$//')
